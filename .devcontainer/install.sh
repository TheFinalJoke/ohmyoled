...
if [ ! -d rpi-rgb-led-matrix ];
then
    git clone $RGBMATRIX_GIT_URL
fi

# Build RGB Matrix 
echo "Install RGB Matrix"
cd rpi-rgb-led-matrix && make build-python PYTHON="$(which python3)" && make install-python PYTHON="$(which python3)" && cd bindings && python3 -m pip install -e python/ -I
