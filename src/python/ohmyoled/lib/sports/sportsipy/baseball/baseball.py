...
class BaseballSportsipy(Runner):
    ...
    async def run(self) -> SportsipyApiResult:
        try:
            ...
            sport["team"] = asyncio.create_task(
                self.run_team(team.shorthand), name="team_task"
            )
            sport["schedule"] = asyncio.create_task(
                self.run_schedule(team.shorthand), name="schedule_task"
            )
            ...
            baseball_result = base.ModuleResult(
                ...
                schedule=[
                    base.Game(
                        ...
                        score=base.Score(
                            team=game.runs_scored, opposing_team=game.runs_allowed
                        ),
                    )
