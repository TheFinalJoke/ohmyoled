...
class WeatherMatrix(Matrix):
    ...
    def nonasync_poll(self):
        try:
            ...
            #     return None
            result = self.api.run_weather_with_asyncio() # type: ignore
            return base.Weather(
                api=result.get_api,
                location=result.get_lat_long,
                location_name=result.get_place,
    ...
    def render_temp(self, api) -> None:
        ...
        self.draw_text(
            (10, 8),
            f"{str(int(api.current.temp))}F",
            font=font,
            fill=self.get_temp_color(int(api.current.temp)),
        )
        self.draw_text((30, 8), "R:", font=font)
        self.draw_text(
            (40, 8),
            f"{str(int(api.current.feels_like))}F",
            font=font,
            fill=self.get_temp_color(int(api.current.feels_like)),
        )
        self.draw_text((1, 18), f"H:", font=font)
        self.draw_text(
            (10, 18),
            f"{str(int(api.dayforcast.todayhigh))}F",
            font=font,
            fill=self.get_temp_color(int(api.dayforcast.todayhigh)),
        )
        self.draw_text((30, 18), f"L:", font=font)
        self.draw_text(
            (40, 18),
            f"{str(int(api.dayforcast.todaylow))}F",
            font=font,
            fill=self.get_temp_color(int(api.dayforcast.todaylow)),
        )
    ...
    def render_humidity(self, api: base.Weather) -> None:
        ...
        self.draw_text(
            (34, 8),
            f"{int(api.current.perciptation_chance)}%",
            font,
            fill=(7, 250, 246),
        )
    ...
    def render_time(self, api) -> None:
        ...
        self.draw_text(
            (1, 18),
            "\uf058",
            font=ImageFont.truetype("/usr/share/fonts/weathericons.ttf", 11),
            fill=(255, 255, 0),
        )
    ...
    def render_conditions(self, api, xpos: int) -> None:
        self.draw_text(
            (-xpos, 26),
            f"Conditions: {api.current.conditions}",
            font=ImageFont.truetype("/usr/share/fonts/04B_03B_.TTF", 8),
            fill=(255, 255, 255),
        )
