...
class SportMatrix(Matrix):
    ...
    def check_offseason(self, api) -> bool:
        try:
            start_time, end_time = (
                api.schedule.positions[0].timestamp,
                api.schedule.positions[-1].timestamp,
            )
    ...
    def build_standings_image(self, api, xpos) -> Tuple[int, int]:
        ...
        text = " ".join(api)
        standings_draw.text((-xpos, 0), text, font=scrolling_font, fill=color)
        ...
        try:
            ...
            if self.check_offseason(api):
                ...
                while xpos < 2700:
                    ...
                    if xpos_for_top == 100:
                        xpos_for_top = 0
                    time.sleep(3) if xpos == 1 else time.sleep(0.001)
            else:
                font = ImageFont.truetype("/usr/share/fonts/fonts/04b24.otf", 14)
                self.draw_multiline_text(
                    (0, 0), f"{api.sport.name}\nOffseason", font=font
                )
                await self.render_image()
                time.sleep(30)
